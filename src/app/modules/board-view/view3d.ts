...
export class View3d extends ViewBase {
  ...
  private forwardDamper = new DelayedValue(300, 0, LinearInterpolator);
  private sideDamper = new DelayedValue(300, 0, LinearInterpolator);
  ...
  constructor(
    ...
    private state: Map<string, Source<any>>,
    // private gridController: GridController,
    // private art: Map<number, ArtInfo>
  ) {
    super(glCtx.gl, glCtx.offscreen, canvas);
  ...
  protected mouse(mx: number, my: number) {
    this.control.track(mx, my, this.state.get('lookaim').get());
  }
  ...
  private move(dt: number) {
    ...
    const cameraSpeed = state.get('camera_speed').get();
