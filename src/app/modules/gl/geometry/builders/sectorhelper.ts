...
export function updateSectorHelper(cache: BuildRenderableProvider, ctx: RenderablesCacheContext, secId: number, builder: SectorHelperBuilder): SectorHelperBuilder {
  ...
  const pointTex = ctx.textures.get(-1);
  const board = ctx.board();
  ...
  fillBuffersForSectorWireframe(secId, sec, sec.ceilingstat.slopped ? sec.ceilingheinum : 0, sec.ceilingz, board, builder.ceilwire);
  fillBuffersForSectorWireframe(secId, sec, sec.floorstat.slopped ? sec.floorheinum : 0, sec.floorz, board, builder.floorwire);
  ...
  builder.ceilgrid.solid = sectorRenderable.ceiling as SolidBuilder;

  mat4.copy(builder.floorgrid.gridTexMat, GRID_SECTOR_MATRIX);
  builder.floorgrid.solid = sectorRenderable.floor as SolidBuilder;
