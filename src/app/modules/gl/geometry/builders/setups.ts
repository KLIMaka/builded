...
import { BuildBuffer, GenericBuildBuffer } from "../../buffers";
...
export class GenericBufferSetup implements StateSetup {
  ...
  createDrawCall(kind: number): DrawCall {
    const hint = hash(this.shaderName, this.buff, this.textureHint(), this.offset);
    return new DrawCall([...this.values], this.shaderName, this.buff, this.offset, this.size, this.mode, hint, kind);
  }


  textureHint() { return null }
  public shader(shader: string) { this.shaderName = shader; return this }
...
export class SolidSetup extends BufferSetup {
  ...
  protected visIdx: number;
  protected modelMatrixIdx: number;
  ...
  constructor(state: State) {
    ...
    this.visIdx = this.register('vis', state);
    this.modelMatrixIdx = this.register('M', state);
  }
  ...
  public vis(vis: number) { this.values.set(this.visIdx, vis); return this }
  public modelMat(mat: mat4) { this.values.set(this.modelMatrixIdx, mat); return this }
...
export abstract class BufferRenderable<T extends BufferSetup> implements Builder {
  ...
  protected abstract applySetup(setup: T): void;
  abstract reset(): void;
