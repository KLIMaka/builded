...
export function samePicnumWalls(board: Board, wallId: number) {
  ...
  const visitor = (wallId: number) => {
    ...
    if (nextsectorId !== -1) {
      const cz = slope(board, sectorId, wall.x, wall.y, true);
      const fz = slope(board, sectorId, wall.x, wall.y, false);
      const cnz = slope(board, nextsectorId, wall.x, wall.y, true);
      const fnz = slope(board, nextsectorId, wall.x, wall.y, false);
