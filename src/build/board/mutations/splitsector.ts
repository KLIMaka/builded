...
function pointEq(p1: point2d, p2: point2d): boolean {
...
function splitSectorImpl<B extends Board>(board: B, sectorId: number, firstWall: number, lastWall: number, start: number, points: Collection<point2d>, refs: BuildReferenceTracker, api: EngineApi<B>) {
  const WALL_MAPPER = (w: number) => api.cloneWall(board.walls[w]);
  ...
  for (const lid of loopPoints(board, sectorId)) {
    ...
    else (loopInPolygon(board, lid, loopPoly) ? newSectorBuilder : oldSectorBuilder).addLoop(iter(loopWalls(board, lid)).map(WALL_MAPPER));
  }
  ...
  oldSectorBuilder
    ...
    .addWalls(iter(wallsBetween(board, firstWall, lastWall)).map(WALL_MAPPER))
    .loop();
  oldSectorBuilder.build(board, sectorId, refs);
  return newSectorId;
}

const POINT_MAPPER = (w: Wall) => pair(w.x, w.y);
