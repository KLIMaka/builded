...
function bitmask(bit: number) { return 1 << bit }
function addr(idx: number): [number, number] { return [int(idx / 8), idx % 8] }
...
export class Bitvec {
  ...
  set(idx: number, value: boolean): void {
    this.ensureSize(idx);
    const [byte, bit] = addr(idx);
    if (value) this.arr[byte] |= bitmask(bit)
    else this.arr[byte] &= ~bitmask(bit)
