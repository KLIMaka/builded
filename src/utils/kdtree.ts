...
export class KDTree {
  ...
  public closest(pos: [number, number]): [number, number, number] {
    const closestNode = this.findClosest(pos, this.top, Number.MAX_VALUE, 0);
    return [this.tree[closestNode], this.tree[closestNode + 1], this.tree[closestNode + 2]];
  }

  private findClosest(pos: [number, number], node: number, mind: number, depth: number): number {
    const p = this.points[this.tree[node]];
    const d = len2d(p[0] - pos[0], p[1] - pos[1]);
    if (this.tree[node + 1] == -1 && this.tree[node + 2] == -1) return d < mind ? node : -1;
    const z = depth & 1;
    const dz = pos[z] - p[z];
    ...
    if (Math.abs(dz) < d && nextNextNode != -1) {
      const closest = this.findClosest(pos, nextNextNode, d, depth + 1);
      if (closest != -1) return closest;
    }
